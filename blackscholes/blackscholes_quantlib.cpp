...
public:
   BlackScholesPricer(bool call, double price, double strike, double tau,
   double r, double fr, double vol);
...
BlackScholesPricer::BlackScholesPricer(bool call, double price, double
strike, double tau, double r, double fr, double vol)
:m_price(price),
m_strike(strike),
m_tau(tau),
m_rate(r),
...
m_isCall(call)
{
   boost::shared_ptr<QuantLib::PlainVanillaPayoff>
      m_option (new QuantLib::PlainVanillaPayoff(call ?
      QuantLib::Option::Call : QuantLib::Option::Put, strike));
   ...
   m_calc.reset(new QuantLib::BlackScholesCalculator(m_option, m_price,
   for_disc, stdev, cur_disc));
}
